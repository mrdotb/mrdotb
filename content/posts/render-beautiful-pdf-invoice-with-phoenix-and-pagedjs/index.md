...
Now we can generate a new Phoenix project. I will disable a lot of generator features because it's not needed here.
...
The context will also mock the invoices.
...
Lets download and add pagedjs to our vendor.

```bash
curl https://unpkg.com/pagedjs@0.2.0/dist/paged.js --output assets/vendor/paged.js
```

You can use npm if you prefer the name of the package is ... `pagedjs`

## Step 5: Create the pdf layout

Since we only need pagedjs on the page where we will show our pdf preview we are going to create a js file and a css that will only be called if the root pdf layout is used.
We also need a small ui that will wrap the preview generated by pagedjs.
